...
def initiate_scan(
    ...
    try:
        ...
        http_url = (
            f"{domain.name}{starting_point_path}"
...
def initiate_subscan(
    ...
    # Send start notif
    send_scan_notif.delay(
        scan.id, subscan_id=subscan.id, engine_id=engine_id, status="RUNNING"
    )
    ...
    base_url = (
        f"{subdomain.name}{starting_point_path}"
        if starting_point_path
        else subdomain.name
    )
    endpoint, _ = save_endpoint(
        base_url, crawl=enable_http_crawl, ctx=ctx, subdomain=subdomain
    )
    if endpoint and endpoint.is_alive:
        # TODO: add `root_endpoint` property to subdomain and simply do
        # subdomain.root_endpoint = endpoint instead
...
@app.task(
    name="subdomain_discovery", queue="main_scan_queue", base=ReconpointTask, bind=True
)
def subdomain_discovery(self, host=None, ctx=None, description=None):
    ...
    if self.starting_point_path:
        logger.warning(
            f"Ignoring subdomains scan as an URL path filter was passed ({self.starting_point_path})."
        )
        return
    ...
    default_subdomain_tools = [
        tool.name.lower()
        for tool in InstalledExternalTool.objects.filter(is_default=True).filter(
            is_subdomain_gathering=True
        )
    ]
    custom_subdomain_tools = [
        tool.name.lower()
        for tool in InstalledExternalTool.objects.filter(is_default=False).filter(
            is_subdomain_gathering=True
        )
    ]
    ...
    for tool in tools:
        ...
        if tool in default_subdomain_tools:
            if tool == "amass-passive":
                ...
                cmd = (
                    f"python3 /usr/src/github/ctfr/ctfr.py -d {host} -o {results_file}"
                )
                ...
                cmd_extract = (
                    rf"grep -oE '([a-zA-Z0-9]([-a-zA-Z0-9]*[a-zA-Z0-9])?\.)+{host}'"
                )
            ...
            if "{TARGET}" not in cmd:
                logger.error(
                    f"Missing {{TARGET}} placeholders in {tool} configuration. Skipping."
                )
                continue
            if "{OUTPUT}" not in cmd:
                logger.error(
                    f"Missing {{OUTPUT}} placeholders in {tool} configuration. Skipping."
                )
                continue
            ...
            cmd = (
                cmd.replace("{PATH}", custom_tool.github_clone_path)
                if "{PATH}" in cmd
                else cmd
            )
        else:
...
def screenshot(self, ctx={}, description=None):
    ...
    if not os.path.isfile(output_path):
        logger.error(
            f"Could not load EyeWitness results at {output_path} for {self.domain.name}."
        )
        return

    # Loop through results and save objects in DB
    screenshot_paths = []
    required_cols = ["Protocol", "Port", "Domain", "Request Status", "Screenshot Path"]
...
def port_scan(self, hosts=[], ctx={}, description=None):
    ...
    if hosts:
        ...
        hosts = get_subdomains(
            write_filepath=input_file, exclude_subdomains=exclude_subdomains, ctx=ctx
        )
    ...
    for line in stream_command(
        ...
        subdomain = Subdomain.objects.filter(
            name=host, target_domain=self.domain, scan_history=self.scan
        ).first()
        ...
        if port_number not in [80, 443]:
            ...
            endpoint, _ = save_endpoint(
                http_url, crawl=enable_http_crawl, ctx=ctx, subdomain=subdomain
            )
            if endpoint:
...
def waf_detection(self, ctx={}, description=None):
    ...
    for line in wafs:
        ...
        waf_manufacturer = (
            waf_info[waf_info.find("(") + 1 : waf_info.find(")")]
            .strip()
            .replace(".", "")
        )
        ...
        # Add waf to db
        waf, _ = Waf.objects.get_or_create(name=waf_name, manufacturer=waf_manufacturer)
...
def dir_file_fuzz(self, ctx={}, description=None):
    ...
    cmd += (
        f" -recursion -recursion-depth {recursive_level} "
...
def fetch_url(self, urls=[], ctx={}, description=None):
    ...
    tasks = group(
        run_command.si(
            cmd, shell=True, scan_id=self.scan_id, activity_id=self.activity_id
        )
        for tool, cmd in cmd_map.items()
        if tool in tools
    )
    ...
    for gf_pattern in gf_patterns:
        ...
        if not os.path.exists(gf_output_file):
            logger.error(
                f'Could not find GF output file {gf_output_file}. Skipping GF pattern "{gf_pattern}"'
            )
            continue
        ...
        for url in lines:
            ...
            endpoint, created = save_endpoint(
                http_url, crawl=False, subdomain=subdomain, ctx=ctx
            )
            if not endpoint:
                continue
            ...
            pattern = (
                f"{earlier_pattern},{gf_pattern}" if earlier_pattern else gf_pattern
            )
...
@app.task(
    name="vulnerability_scan", queue="main_scan_queue", bind=True, base=ReconpointTask
)
def vulnerability_scan(self, urls=[], ctx={}, description=None):
    ...
    if should_run_nuclei:
        _task = nuclei_scan.si(urls=urls, ctx=ctx, description=f"Nuclei Scan")
        grouped_tasks.append(_task)

    if should_run_crlfuzz:
        _task = crlfuzz_scan.si(urls=urls, ctx=ctx, description=f"CRLFuzz Scan")
        grouped_tasks.append(_task)

    if should_run_dalfox:
        _task = dalfox_xss_scan.si(urls=urls, ctx=ctx, description=f"Dalfox XSS Scan")
        grouped_tasks.append(_task)

    if should_run_s3scanner:
        _task = s3scanner.si(ctx=ctx, description=f"Misconfigured S3 Buckets Scanner")
        grouped_tasks.append(_task)
...
@app.task(
    name="nuclei_individual_severity_module",
    queue="main_scan_queue",
    base=ReconpointTask,
    bind=True,
)
def nuclei_individual_severity_module(
    self,
    cmd,
    severity,
    enable_http_crawl,
    ...
    for line in stream_command(
        ...
        subdomain, _ = Subdomain.objects.get_or_create(
            name=subdomain_name, scan_history=self.scan, target_domain=self.domain
        )
        ...
        if record_exists(
            Vulnerability, data=vuln_data_copy, exclude_keys=object_comparison_exclude
        ):
            logger.warning(
        ...
        httpx_crawl = (
            False if response else enable_http_crawl
        )  # avoid yet another httpx crawl
        endpoint, _ = save_endpoint(
            http_url, crawl=httpx_crawl, subdomain=subdomain, ctx=ctx
        )
        if endpoint:
            http_url = endpoint.http_url
            if not httpx_crawl:
        ...
        send_report = (
            ...
            and api_key_check_query.exists()
    ...
    if should_fetch_gpt_report and OpenAiAPIKey.objects.all().first():
        ...
        vulns = (
            Vulnerability.objects.filter(scan_history__id=self.scan_id)
            .filter(source=NUCLEI)
            .exclude(severity=0)
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            future_to_gpt = {
                executor.submit(get_vulnerability_gpt_report, vuln): vuln
...
def get_vulnerability_gpt_report(vuln):
    ...
    stored = (
        GPTVulnerabilityReport.objects.filter(url_path=path).filter(title=title).first()
    )
    if stored and stored.description and stored.impact and stored.remediation:
        ...
        vulnerability_description = get_gpt_vuln_input_description(title, path)
...
@app.task(
    name="dalfox_xss_scan", queue="main_scan_queue", base=ReconpointTask, bind=True
)
def dalfox_xss_scan(self, urls=[], ctx={}, description=None):
    ...
    user_agent = dalfox_config.get(USER_AGENT) or self.yaml_configuration.get(
        USER_AGENT
    )
    ...
    if urls:
        ...
        get_http_urls(
            is_alive=False, ignore_files=False, write_filepath=input_path, ctx=ctx
        )
    ...
    for line in stream_command(
        ...
        endpoint, _ = save_endpoint(http_url, crawl=True, subdomain=subdomain, ctx=ctx)
        if endpoint:
            http_url = endpoint.http_url
            endpoint.save()
...
def crlfuzz_scan(self, urls=[], ctx={}, description=None):
    ...
    if urls:
        ...
        get_http_urls(
            is_alive=False, ignore_files=True, write_filepath=input_path, ctx=ctx
        )
    ...
    for provider in providers:
        ...
        for line in stream_command(
            ...
            if line.get("bucket", {}).get("exists", 0) == 1:
                ...
                logger.info(
                    f"s3 bucket added {result['provider']}-{result['name']}-{result['region']}"
                )
...
def http_crawl(
    ...
    # Run command
    cmd += (
        f" -cl -ct -rt -location -td -websocket -cname -asn -cdn -probe -random-agent"
    )
    ...
    for line in stream_command(
        ...
        endpoint.save()
        endpoint_str = (
            f"{http_url} [{http_status}] `{content_length}B` `{webserver}` `{rt}`"
        )
        logger.warning(endpoint_str)
        if endpoint and endpoint.is_alive and endpoint.http_status != 403:
            self.notify(
                fields={"Alive endpoint": f"• {endpoint_str}"}, add_meta_info=False
            )
        ...
        for ip_address in a_records:
            ip, created = save_ip_address(
                ip_address, subdomain, subscan=self.subscan, cdn=cdn
            )
        ...
        if host:
            ip, created = save_ip_address(
                host, subdomain, subscan=self.subscan, cdn=cdn
            )
    ...
    if should_remove_duplicate_endpoints:
        ...
        remove_duplicate_endpoints(
            self.scan_id, self.domain_id, self.subdomain_id, filter_ids=endpoint_ids
        )
...
def send_hackerone_report(vulnerability_id):
    ...
    api_key = HackerOneAPIKey.objects.filter(
        username__isnull=False, key__isnull=False
    ).first()

    if not (vulnerability.target_domain.h1_team_handle and hackerone and api_key):
        logger.error(
            "Missing required data: team handle, Hackerone config, or API key."
        )
    ...
    data = {
        "data": {
            ...
            "attributes": {
                ...
                "impact": (
                    "More information about the impact and vulnerability can be found here: \n"
    ...
    for host in hosts:
        ...
        if hostnames_dict:
            ...
            hostnames_list = (
                ...
                if isinstance(hostnames_dict["hostname"], list)
                else [hostnames_dict["hostname"]]
            )
        ...
        for hostname in hostnames:
            ...
            for port in ports:
                ...
                for script in scripts:
                    ...
                    logger.debug(
                        f'Ran nmap script "{script_id}" on {port_number}/{port_protocol}:\n{script_output}\n'
                    )
                    ...
                    # 	url_vulns.extend(vulns)
                        ...
                        logger.warning(
                            f'Script output parsing for script "{script_id}" is not supported yet.'
                        )
...
def parse_dalfox_result(line):
    ...
    description = ""
    description += (
        f" Evidence: {line.get('evidence')} <br>" if line.get("evidence") else ""
    )
    description += (
        f" Message: {line.get('message')} <br>" if line.get("message") else ""
    )
    description += (
        f" Payload: {line.get('message_str')} <br>" if line.get("message_str") else ""
    )
    description += (
        f" Vulnerable Parameter: {line.get('param')} <br>" if line.get("param") else ""
    )
...
def query_whois(target, force_reload_whois=False):
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            futures_func = {
                executor.submit(
                    get_domain_historical_ip_address, target
                ): "historical_ips",
                executor.submit(
                    fetch_related_tlds_and_domains, target
                ): "related_tlds_and_domains",
            ...
            for future in concurrent.futures.as_completed(futures_func):
                ...
                try:
                    ...
                    logger.error(
                        f"An error occurred while fetching {func_name} for {target}: {str(e)}"
                    )
                    continue
        ...
        logger.error(
            f"An error occurred while querying WHOIS information for {target}: {str(e)}"
        )
        return {
            "status": False,
            "target": target,
    ...
    for line in result.splitlines():
        try:
            ...
            if extracted_result.domain == extracted.domain:
                ...
                extracted_result.domain != extracted.domain
                or extracted_result.subdomain
            ):
                related_domains.add(line)
        except Exception as e:
            logger.error(
                f"An error occurred while fetching related TLDs and domains for {domain}: {str(e)}"
            )
            continue

    logger.info(
        f"Found {len(related_tlds)} related TLDs and {len(related_domains)} related domains for {domain}"
    )
...
@app.task(
    name="remove_duplicate_endpoints",
    bind=False,
    queue="remove_duplicate_endpoints_queue",
)
def remove_duplicate_endpoints(
    ...
    endpoints = EndPoint.objects.filter(scan_history__id=scan_history_id).filter(
    ...
    for field_name in duplicate_removal_fields:
        cl_query = (
            endpoints.values_list(field_name)
        ...
        for field_value, count in cl_query:
            if count > DELETE_DUPLICATES_THRESHOLD:
                eps_to_delete = (
                    endpoints.filter(**{field_name: field_value})
...
def extract_httpx_url(line):
    ...
    is_redirect = status_code in REDIRECT_STATUS_CODES or any(
...
def save_metadata_info(meta_dict):
    ...
    if not result:
        logger.error(
            f"No metadata result from Google Search for {meta_dict.osint_target}."
        )
        return []
...
def save_endpoint(http_url, ctx={}, crawl=False, is_default=False, **endpoint_data):
    ...
    if crawl:
        ctx["track"] = False
        results = http_crawl(urls=[http_url], method="HEAD", ctx=ctx)
        if results:
        ...
        endpoints = EndPoint.objects.filter(
            scan_history=scan, target_domain=domain, http_url=http_url, **endpoint_data
        )
...
def save_subdomain(subdomain_name, ctx={}):
    ...
    valid_domain = (
        validators.domain(subdomain_name)
        or validators.ipv4(subdomain_name)
        or validators.ipv6(subdomain_name)
    )
    if not valid_domain:
    ...
    if ctx.get("domain_id"):
        ...
        if domain.name not in subdomain_name:
            logger.error(
                f"{subdomain_name} is not a subdomain of domain {domain.name}. Skipping."
            )
            return None, False
    ...
    subdomain, created = Subdomain.objects.get_or_create(
        scan_history=scan, target_domain=domain, name=subdomain_name
    )
    if created:
...
def llm_vulnerability_description(vulnerability_id):
    ...
    if not path:
        path = "/"
    stored = (
        GPTVulnerabilityReport.objects.filter(url_path=path)
    ...
    if stored and stored.description and stored.impact and stored.remediation:
        ...
        vulnerability_description = get_gpt_vuln_input_description(
            lookup_vulnerability.name, path
        )
    ...
    for vuln in Vulnerability.objects.filter(
        name=lookup_vulnerability.name, http_url__icontains=path
    ):
        vuln.description = response.get("description", vuln.description)
        vuln.impact = response.get("impact")
        vuln.remediation = response.get("remediation")
